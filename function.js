  ...
  restaurant: {
    ...
    lines: [
      ...
      },
      {
        ...
        e: "Please follow me. What would you like?",
        r: true,
      },
      {
        s: "B",
        ...
        e: "I'd like beef noodles and water.",
        r: false,
      },
      {
        s: "A",
  ...
  shopping: {
    ...
    lines: [
      ...
      },
      {
        ...
        e: "Too expensive! Can you lower the price?",
        r: false,
      },
      {
        s: "A",
        ...
        e: "Okay, 180 yuan. That's the lowest.",
        r: true,
      },
      {
        s: "B",
...
function renderPhrasebook() {
  const cats = document.getElementById("phraseCats"),
    grid = document.getElementById("phraseGrid");
...
function loadDialogue(key) {
  ...
  const linesHTML = dlg.lines
    ...
    .join("");
  content.innerHTML = `<div class="dialogue-lines">${linesHTML}</div><button class="dlg-speak-all" onclick="speakDialogue('${key}')">🔊 Listen to Full Conversation</button>`;
}
...
function renderQuiz() {
  if (quizIdx >= quizPool.length) {
    document.getElementById("quizWrap").innerHTML =
      `<div class="quiz-result"><div style="font-size:2.5rem">🎉</div><div class="section-heading" style="margin:12px 0 4px">Quiz Complete!</div><div class="quiz-score-big">${quizScore}/${quizPool.length}</div><div style="color:var(--muted);margin-bottom:20px">${quizScore >= quizPool.length * 0.8 ? "🏆 Excellent work!" : quizScore >= quizPool.length * 0.5 ? "👍 Good effort!" : "📚 Keep practicing!"}</div><button class="next-btn" onclick="restartQuiz()">🔄 Try Again</button></div>`;
    return;
  }
  ...
  document.getElementById("quizWrap").innerHTML =
    `<div class="quiz-question"><div class="quiz-q-hanzi">${q.h}</div><div class="quiz-q-label">What does this mean? — ${q.p}</div></div><div class="quiz-options" id="quizOpts">${opts.map((o) => `<button class="quiz-opt" onclick="checkAnswer(this,'${o.replace(/'/g, "\\'")}','${q.e.replace(/'/g, "\\'")}','${q.h}')">${o}</button>`).join("")}</div><div class="quiz-status">Question ${quizIdx + 1} of ${quizPool.length} &nbsp;|&nbsp; Score: ${quizScore}</div>`;
}
...
function updateStreak() {
  const today = new Date().toDateString(),
    stored = localStorage.getItem("xuexiDate");
